...
> **먼저 밝혀둡니다.** 최종 파이프라인(Gemini 추출 3회 + BM25 대조군)의 평가 수치는
> **아직 없습니다.** OpenAI와 Gemini 양쪽 모두 API 한도/크레딧이 소진된 상태로 끝났습니다.
> 아래 측정 결과표는 **OpenAI로 만든 v1~v3.5까지**이며, 그 구간에서 얻은 결론은 유효합니다.
> 재측정 방법은 §3 끝에 적어두었습니다.
...
| **v4** | **Gemini 3회 + 소문 표시 + BM25 대조군** | — | — | — | — | — | **미측정** | — | — | — |

*v1~v3.5: OpenAI(gpt-5-mini 추출 / gpt-5 답변), basic RAG는 임베딩 검색.*
...
API 크레딧이 복구되면 이 한 줄로 §3 표의 v4 행이 채워집니다.

```bash
python evaluate.py
```

추출 결과는 `output/extract_cache*/`에 캐시돼 있어 **다시 뽑지 않습니다.**
평가에 드는 호출은 13문항 × (답변 2 + 채점 2) ≈ 55회뿐이고,
basic RAG 대조군은 **BM25로 바꿔서 API를 전혀 쓰지 않습니다.**

단, v4는 추출 모델(Gemini)과 대조군 방식(BM25)이 모두 바뀌었으므로
**v1~v3.5와 직접 비교하면 안 됩니다.** 같은 조건에서 비교하려면 v4 이후만 놓고 봐야 합니다.
...
> **아직 없는 캡처:** 질문에 답한 화면(답변 + 탄 경로 + 근거 삼중항).
> LLM 호출이 필요한데 API 크레딧이 소진된 상태라 못 찍었습니다.
> 복구되면 `python scripts/capture.py --with-answer` 로 자동 생성됩니다.
...
- **비용을 계획에 넣지 않았습니다.** 추출 96장 × 3회를 두 제공자에서 반복하다
  양쪽 다 한도를 소진했고, 그래서 최종 측정을 못 끝냈습니다. "고치고 다시 돌린다"를
  몇 번 할지 미리 정해두고 예산을 나눴어야 했습니다. 이번 프로젝트에서 가장 뼈아픈 실수입니다.

### 앞으로 보완하고 싶은 것

1. **최종 측정을 마치는 것** (§3의 v4 행). `python evaluate.py` 한 번이면 됩니다.
2. **소문·부인 판정을 전 회차에 적용** — 이 항목은 3차 추출에서 도입해서,
   1·2차에만 잡힌 관계는 판정이 비어 있습니다. 지금은 "판정도 없고 한 번만 뽑힌 관계는
   확정으로 쓰지 않는다"는 안전장치로 막고 있습니다.
3. **모델 사전 지식 누수를 직접 시험하지 못했습니다.** 지금은 프롬프트로 막고
   골든셋 4문항으로 확인하는 수준입니다. "일부러 유도하는" 공격 문항을 더 넣어 봐야 합니다.
4. **관계도 그림** — PRD에서 "나중에"로 미뤄둔 것. 답변에 쓰인 부분 그래프를 그려주면
   읽기가 훨씬 편할 겁니다.
5. **Event 노드 정규화** — Gemini로 바꾸며 244개→48개로 크게 줄었지만,
   여전히 서술문 형태의 사건 이름이 남아 있어 장이 다르면 안 합쳐집니다.
